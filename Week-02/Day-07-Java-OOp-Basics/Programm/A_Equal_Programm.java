...
return(this.id ==s.id &&this.name ==this.name)if
those are true
then only return true else false
...
hashcode();     //then it give bydefault value
...
equals(s2);    //if this is true then there hashcode must be same
...
calculate hashcode(usually we took id as hashcode) ...if
hashcode is
equal then
we run
equal method
...
Yes,
exactly.Same hashCode
does NOT
mean data
is equal. ✅

hashCode() only helps to find a possible location (bucket).
It is
not the
final comparison.
