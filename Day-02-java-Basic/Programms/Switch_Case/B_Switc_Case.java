...
//Switch is more efficient then if else...in if else we check from first condition to last till we not find exact matching
    ....but switch direct jump on that particular case

//this efficiency of switch case provide by jvm it provide jump table like array if 1 come then
 it direct run first case if 2 occur then case runs there has O(1) complexity
...
//but internally if i has case like 1 1000 10000 then jvm only create 3 size of jump table..
not from 1 to 1000000 type so we achive effectiveness here(Here binary search occurs known as lookup switch)


//so when there has near by near next value in case like case 1 case 2 difference were less
then only jump table creation perform(if i know my number has very less ddiff then we create all case jump table)
